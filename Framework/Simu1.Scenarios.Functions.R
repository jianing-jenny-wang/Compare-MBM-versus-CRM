...
simu2.plot.CRM.MBM.diff.Est <- function(dt.crm = crm.1.Nhat, dt.mbm = mbm.1.Nhat, p1_pB = p1, title = "(A)"){
  
  CRM.LP <- subset(dt.crm, p1 == p1_pB, select = c(p1, p2, Rel.Bias.Nhat_LP, LB95.Rel.Bias.Nhat_LP, UB95.Rel.Bias.Nhat_LP))
  CRM.LP$Methods <- rep("CRM (LP)", nrow(CRM.LP))
  ...
  CRM.Chap$Methods <- rep("CRM (Chapman)", nrow(CRM.Chap))
  ...
  MBM.nb.mvue$Methods <- rep("MBM (MVUE)", nrow(MBM.nb.mvue))
  ...
  MBM.nb.mle$Methods <- rep("MBM (MLE)", nrow(MBM.nb.mle))
  ...
  p <- ggplot(CRM.MBM.given.p1, aes(x = p2, y = Rel.Bias.Nhat, group = Methods, color = Methods)) + 
        ...
        scale_y_continuous(limits = c(0.6,2.2), breaks = c(0.6, 1.0, 1.4, 1.8, 2.2)) +
...
simu2.plot.CRM.MBM.diff.p_m <- function(dt.crm = simu2.crm.1.Nhat, dt.mbm = simu2.mbm.1.Nhat, p1_pB = p1, title = "(A)"){
  CRM.p_m <- subset(dt.crm, p1 == p1_pB, select = c(p1, p2, Mean.p_m_crm, SD.p_m_crm, LB95.p_m_crm, UB95.p_m_crm))
  CRM.p_m$Methods <- rep("CRM", nrow(CRM.p_m))
  ...
  MBM.p_m$Methods <- rep("MBM", nrow(MBM.p_m))
  colnames(MBM.p_m) <- c("p1", "p2", "Mean.p_m", "SD.p_m", "LB95.p_m", "UB95.p_m", "Methods")
  
  CRM.MBM.p_m <- as.data.frame(rbind(CRM.p_m, MBM.p_m))
  CRM.MBM.p_m$Rel.Bias.p_m <- CRM.MBM.p_m$Mean.p_m/CRM.MBM.p_m$p1
  CRM.MBM.p_m$LB95.Rel.Bias.p_m <- CRM.MBM.p_m$LB95.p_m/CRM.MBM.p_m$p1
  CRM.MBM.p_m$UB95.Rel.Bias.p_m <- CRM.MBM.p_m$UB95.p_m/CRM.MBM.p_m$p1
  
  p <- ggplot(CRM.MBM.p_m, aes(x = p2, y = Rel.Bias.p_m, group = Methods, color = Methods)) + 
    geom_ribbon(aes(ymin = LB95.Rel.Bias.p_m, ymax = UB95.Rel.Bias.p_m), linetype=2, alpha=0.1) +
    geom_line(aes(y = Rel.Bias.p_m), size = 0.5) +
    geom_point(aes(y = Rel.Bias.p_m)) +
    geom_hline(aes(yintercept=1), color = "black",
               linetype="dashed", size=0.5) +
    scale_y_continuous(limits = c(0.4,1.6), breaks = c(0.4, 0.8, 1.2, 1.6)) +
    ylab(expression(hat(p[1])/p[1])) + 
    xlab("Sampling Probability of Sample 2") +
    ggtitle(title) +
    theme_classic() +
...
simu2.plot.CRM.MBM.p1.to.p2 <- function(dt.crm = crm.1.Nhat, dt.mbm = mbm.1.Nhat, p1_pB = p1){
  ...
  MBM.nb.mvue$Methods <- rep("MBM_MVUE", nrow(MBM.nb.mvue))
  ...
  MBM.nb.mle$Methods <- rep("MBM_MLE", nrow(MBM.nb.mle))
  ...
  p <- ggplot(CRM.MBM.given.p1, aes(x = Ratio.p1.p2, y = Rel.Bias.Nhat, group = Methods, color = Methods)) + 
    ...
    geom_hline(aes(yintercept=1), color = "black",
               linetype="dashed", size=0.5) +
    scale_x_continuous(breaks = c(0, 1, 2, 3 , 6, 12)) +
    ylab("Nhat/N") + 
    xlab("Ratio of source coverage (p1 to p2)") +
    ggtitle("Plot for ratio of p1 to p2") +
    theme_classic() +
