...
t.test.2side_Chap_Strata <- t.test(Nhat_Chap_Strata, mu = pplsize)
pvalue.t.2side_Chap_Strata <- round(t.test.2side_Chap_Strata$p.value, digits = 5)
t.test.gtr_Chap_Strata <- t.test(Nhat_Chap_Strata, mu = pplsize, alternative = "greater")
pvalue.t.gtr_Chap_Strata <- round(t.test.gtr_Chap_Strata$p.value, digits = 5)
t.test.less_Chap_Strata <- t.test(Nhat_Chap_Strata, mu = pplsize, alternative = "less")
pvalue.t.less_Chap_Strata <- round(t.test.less_Chap_Strata$p.value, digits = 5)
...
df <- data.frame(# LP estimator
                ...
                pvalue.t.2side_Chap_Strata, pvalue.t.gtr_Chap_Strata, pvalue.t.less_Chap_Strata, Mean.Nhat_Chap_Strata_Min.m, 
...
mbm.1 <- function(simsize = nsim, pplsize = targetN, p.X1 = scenarios1$p.X1[2], pB.X0 = scenarios1$pB.X0[2], pB.X1 = scenarios1$pB.X1[2], p2 = scenarios1$p2[2]){
  ...
  # Mean.Nhat_MBM_unlk_mle_Min.m <- mean(Nhat_MBM_unlk_mle[which(m == Min.m)])
  ...
  # Mean.Nhat_MBM_unlk_mvue_Min.m <- mean(Nhat_MBM_unlk_mvue[which(m == Min.m)])
...
simu4.plot.CRM.MBM.diff.Est <- function(dt.crm = simu4.crm.1.Nhat, dt.mbm = simu4.mbm.1.Nhat, p1_pB = pB.X1.vec[4]){
  ...
  CRM.Chap_Strata$Methods <- rep("CRM_Chap_Strata", nrow(CRM.Chap_Strata))
  ...
  # MBM.unlk_mle <- subset(dt.mbm, pB.X1 == p1_pB, select = c(pB.X1, p2, Rel.Bias.Nhat_MBM_unlk_mle, LB95.Rel.Bias.Nhat_MBM_unlk_mle, UB95.Rel.Bias.Nhat_MBM_unlk_mle))
  # MBM.unlk_mle$Methods <- rep("MBM_unlk_mle", nrow(MBM.unlk_mle))
  # colnames(MBM.unlk_mle) <- c("p1.X1", "p2", "Rel.Bias.Nhat", "LB95.Rel.Bias.Nhat", "UB95.Rel.Bias.Nhat","Methods")
  # MBM.unlk_mvue <- subset(dt.mbm, pB.X1 == p1_pB, select = c(pB.X1, p2, Rel.Bias.Nhat_MBM_unlk_mvue, LB95.Rel.Bias.Nhat_MBM_unlk_mvue, UB95.Rel.Bias.Nhat_MBM_unlk_mvue))
  # MBM.unlk_mvue$Methods <- rep("MBM_unlk_mvue", nrow(MBM.unlk_mvue))
  # colnames(MBM.unlk_mvue) <- c("p1.X1", "p2", "Rel.Bias.Nhat", "LB95.Rel.Bias.Nhat", "UB95.Rel.Bias.Nhat","Methods")
  # 
  CRM.MBM.given.p1 <- as.data.frame(rbind(CRM.LP, CRM.Chap, CRM.Chap_Strata, MBM.lk_mle, MBM.lk_mvue))   # , MBM.unlk_mle, MBM.unlk_mvue
  ...
  p <- ggplot(CRM.MBM.given.p1, aes(x = p2, y = Rel.Bias.Nhat, group = Methods, color = Methods)) + 
    ...
    ggtitle(paste("(Sim4) CRM vs MBM (N = 5000)--Plot for", "pB(X=1) =", p1_pB, sep = " ")) +
    theme_classic() +
